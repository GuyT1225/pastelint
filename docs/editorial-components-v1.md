...
Compare defaults to Previous engine behavior versus Current verified behavior. Replay supports Step through the repair, Pause, Start over, Previous, Next, Show source, Show repaired result, descriptive progress, polite status, and metadata disclosure. Step through the repair visibly traverses all three states at a two-second interval and stops at current behavior; all manual state changes remain immediate. Native disabled states prevent Previous on the first step, Next on the last step, and Pause while idle. Controls use the publication sans-serif face and are grouped into primary playback, secondary navigation, and quieter view actions. The future article should default to the previous-versus-current Compare presentation, with Replay available as the optional guided action labeled Step through the repair. The QA fixture continues to expose every control; article navigation and verification details may remain quieter without requiring a variant system or changing evidence. The explicit Show repaired result control carries the completion event because no supported programmatic StatsKit event API exists locally; starting playback is not counted as completion.
